    ...
    def __init__(self, action: str):
        ...
        try:
            ...
            if prompt_tree and hasattr(prompt_tree, 'current_directory'):
                ...
                # Fallback to home directory if no current directory is set
                start_dir = os.path.expanduser("~")
                self.notify(f"No current directory set, using home: {start_dir}", severity="information")
        except Exception:
            # Fallback to home directory if no prompt tree found
            start_dir = os.path.expanduser("~")
            self.notify(f"No prompt tree found, using home: {start_dir}", severity="information")
